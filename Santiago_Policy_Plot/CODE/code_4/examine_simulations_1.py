...
def run_program_from_shell_command(total_population = 50000, num_of_simulation = 2 ** 12, output_file_name = "output.csv", horizon = 30, testing_action_step = 10, R0_value = 2.0, I_ratio = 0.0056, R_ratio = 0.0051):
    # Shell command space.
    shell_command = "./pomcp --problem covid --outputfile {} --total_population {} --horizon {} --maximum_testing_number 500 --R0_value {:.2f} --I_class_ratio {:.4f} --R_class_ratio {:.4f} " \
...
if __name__=="__main__":
    ...
    for I_ratio in [0.005, 0.01, 0.05]:
        
        R_ratio = I_ratio * 3.0
        print("Running on population: {}, num_simulation: {}, horizon: {}, testing_action_step: {}, R0_value: {:.2f}, I_ratio: {:.4f}, R_ratio: {:.4f}.".format(
            fixed_population,
        ...
        output_file_name = "output_I_ratio_{:.4f}.csv".format(I_ratio)


        run_program_from_shell_command(total_population=fixed_population, num_of_simulation=fixed_number_of_simulation,
                                       output_file_name=output_file_name, horizon=fixed_horizon, testing_action_step=fixed_testing_action_step, R0_value=fixed_R0_value, I_ratio=I_ratio, R_ratio=R_ratio)
