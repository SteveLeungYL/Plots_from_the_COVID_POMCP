...
for testing_number in range(100, 1001, 100):
    
    data_name = "output_testing_number_{}.csv.process_all.csv".format(
        testing_number
    )
...
for fix_testing_number in [-1, 0, 100, 200]:
    ...
    for testing_number in range(100, 1001, 100):
        ...
        for run_count in range(sample_total_run_count):
            ...
            current_sample_data = current_sample_data[current_sample_data['total_testing_number'] == testing_number]
...
advantages_random = (mean_last_cum_I_array[10:20] - mean_last_cum_I_array[0:10]) / mean_last_cum_I_array[10:20]
advantages_pure_symp = (mean_last_cum_I_array[20:30] - mean_last_cum_I_array[0:10]) / mean_last_cum_I_array[20:30]
advantages_half_half = (mean_last_cum_I_array[30:40] - mean_last_cum_I_array[0:10]) / mean_last_cum_I_array[30:40]
advantages_pure_asymp = (mean_last_cum_I_array[40:50] - mean_last_cum_I_array[0:10]) / mean_last_cum_I_array[40:50]
...
all_advantages_random = (last_day_active_I_array[10:20] - last_day_active_I_array[0:10]) / last_day_active_I_array[10:20]
all_advantages_pure_symp = (last_day_active_I_array[20:30] - last_day_active_I_array[0:10]) / last_day_active_I_array[20:30]
all_advantages_half = (last_day_active_I_array[30:40] - last_day_active_I_array[0:10]) / last_day_active_I_array[30:40]
all_advantages_pure_asymp = (last_day_active_I_array[40:50] - last_day_active_I_array[0:10]) / last_day_active_I_array[40:50]
...
plt.bar(x, min_advantages, tick_label = xlabel)
ax.errorbar(x, min_advantages, yerr = upper_min_advantages, color = "k", capsize=3, fmt=".", ms = 0.0001)

plt.ylim(0, plt.ylim()[1])

plt.ylabel("DOCTOR advantages")
...
# ax.errorbar(x, mean_peak_H_list[3:6], yerr = y_upper_error[0:3], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_peak_H_list[6:9], width=width, label='Half Asymp, Half Symp')
# ax.errorbar(x, mean_peak_H_list[6:9], yerr = y_upper_error[0:3], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_peak_H_list[9:12], width=width, label='Pure Asymp')
# ax.errorbar(x, mean_peak_H_list[9:12], yerr = y_upper_error[0:3], color = "k", capsize=3, fmt=".", ms = 0.0001)
...
plt.bar(x, mean_sum_asymptomatic_testing_list, label='Public Testing Number')
