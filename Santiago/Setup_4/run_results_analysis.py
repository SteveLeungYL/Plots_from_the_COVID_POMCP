...
for I_ratio in [0.003, 0.005, 0.008]:
    
    data_name = "output_I_ratio_{:.4f}.csv.process_all.csv".format(
        I_ratio
    )
...
for fix_testing_number in [-1, 0, 100, 200]:
    ...
    for initial_I_ratio in [0.003, 0.005, 0.008]:
        ...
        for run_count in range(sample_total_run_count):
            ...
            current_sample_data = current_sample_data.round(3)
            current_sample_data = current_sample_data[current_sample_data['initial_I_ratio'] == initial_I_ratio]
...
ax.errorbar(x, mean_last_cum_I_list[0:3], yerr = y_upper_error[0:3], color = "k", capsize=9, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[3:6], width=width, label='Random', tick_label = xlabel)
ax.errorbar(x, mean_last_cum_I_list[3:6], yerr = y_upper_error[3:6], color = "k", capsize=9, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[6:9], width=width, label='Pure Symp')
ax.errorbar(x, mean_last_cum_I_list[6:9], yerr = y_upper_error[6:9], color = "k", capsize=9, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[9:12], width=width, label='Half Asymp, Half Symp')
ax.errorbar(x, mean_last_cum_I_list[9:12], yerr = y_upper_error[9:12], color = "k", capsize=9, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[12:15], width=width, label='Pure Asymp')
ax.errorbar(x, mean_last_cum_I_list[12:15], yerr = y_upper_error[12:15], color = "k", capsize=9, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

plt.legend(loc = 4, ncol = 2)

plt.xlabel(r'Asymptomatic rate $\bf{\phi}$', fontsize=35)
plt.ylabel(r'% improvements', fontsize=35)

plt.tight_layout()

plt.savefig("./Plots/cumulative_I_curve.png", dpi = 400)
plt.close(fig)
...
ax.errorbar(x, min_advantages, yerr = upper_min_advantages, color = "k", capsize=12, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)

plt.ylim(0, plt.ylim()[1])

plt.xlabel(r'Infection ratio $|\bf{I} | / \bf{N}$', fontsize=35)
plt.ylabel(r'% improvements', fontsize=35)
