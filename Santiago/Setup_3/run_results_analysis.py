...
for R0_value in [1.0, 3.0, 5.0]:
    
    data_name = "output_R0_value_{:.2f}.csv.process_all.csv".format(
        R0_value
    )
    ...
    mean_asymptomatic_testing_list.append(mean_asymptomatic_testing_curve[1:])
    upper_asymptomatic_testing_list.append(upper_asymptomatic_testing_curve[1:])
    lower_asymptomatic_testing_list.append(lower_asymptomatic_testing_curve[1:])

    mean_symptomatic_testing_list.append(mean_symptomatic_testing_curve[1:])
    upper_symptomatic_testing_list.append(upper_symptomatic_testing_curve[1:])
    lower_symptomatic_testing_list.append(lower_symptomatic_testing_curve[1:])
...
for fix_testing_number in [-1, 0, 100, 200]:
    ...
    for R0_value in [1.0, 3.0, 5.0]:
        ...
        for run_count in range(sample_total_run_count):
            ...
            current_sample_data = current_sample_data[current_sample_data['R0_value'] == R0_value]
...
# plt.ylabel("Last Day Active I")

# plt.savefig("./Plots/active_I_curve_Linear.png")
# plt.close(fig)
...
# x = list(range(len(mean_last_cum_I_list[0:3])))
# plt.bar(x, mean_last_cum_I_list[0:3], width=width, label='POMCP', log=True)
# ax.errorbar(x, mean_last_cum_I_list[0:3], yerr = y_upper_error[0:3], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_last_cum_I_list[3:6], width=width, label='Random', tick_label = xlabel, log=True)
# ax.errorbar(x, mean_last_cum_I_list[3:6], yerr = y_upper_error[3:6], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_last_cum_I_list[6:9], width=width, label='Pure Symp', log=True)
# ax.errorbar(x, mean_last_cum_I_list[6:9], yerr = y_upper_error[6:9], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_last_cum_I_list[9:12], width=width, label='Half Asymp, Half Symp', log=True)
# ax.errorbar(x, mean_last_cum_I_list[9:12], yerr = y_upper_error[9:12], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_last_cum_I_list[12:15], width=width, label='Pure Asymp', log=True)
# ax.errorbar(x, mean_last_cum_I_list[12:15], yerr = y_upper_error[12:15], color = "k", capsize=3, fmt=".", ms = 0.0001)
...
# plt.savefig("./Plots/active_I_curve_Log.png")
# plt.close(fig)
...
# Plot the best baseline advantages
...
min_advantages = []
for i in range(3):
    min_advantages.append( min(advantages_random[i], advantages_pure_symp[i], advantages_half_half[i], advantages_pure_asymp[i]) )
...
upper_min_advantages = []

for i in range(3):

    mean_advantages_ran, upper_advantages_ran, _ = mean_confidence_interval(all_advantages_random[i])
    mean_advantages_sy, upper_advantages_sy, _ = mean_confidence_interval(all_advantages_pure_symp[i])
    mean_advantages_ha, upper_advantages_ha, _ = mean_confidence_interval(all_advantages_half[i])
    mean_advantages_as, upper_advantages_as, _ = mean_confidence_interval(all_advantages_pure_asymp[i])

    minimum_value = min(mean_advantages_ran, mean_advantages_sy, mean_advantages_ha, mean_advantages_as)
    
    if minimum_value == mean_advantages_ran:
        upper_min_advantages.append(upper_advantages_ran)
    elif minimum_value == mean_advantages_sy:
        upper_min_advantages.append(upper_advantages_sy)
    elif minimum_value == mean_advantages_ha:
        upper_min_advantages.append(upper_advantages_ha)
    else:
        upper_min_advantages.append(upper_advantages_as)

upper_min_advantages = np.array(upper_min_advantages) - np.array(min_advantages)
...
x = list(range(len(min_advantages)))
plt.bar(x, min_advantages, tick_label = xlabel, hatch = patterns[2])
ax.errorbar(x, min_advantages, yerr = upper_min_advantages, color = "k", capsize=13, fmt=".", ms = 0.0001, elinewidth=5, capthick = 2)


plt.xlabel(r'$\bf{R_0}$', fontsize=35)
plt.ylabel(r'% improvements', fontsize=35)

plt.legend(["Best Performance Baseline"])
...
# plt.bar(x, mean_peak_H_list[0:3], width=width, label='POMCP', log=True)
# ax.errorbar(x, mean_peak_H_list[0:3], yerr = y_upper_error[0:3], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_peak_H_list[3:10], width=width, label='Pure Symp', tick_label = xlabel, log=True)
# ax.errorbar(x, mean_peak_H_list[3:6], yerr = y_upper_error[3:6], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_peak_H_list[6:9], width=width, label='Half Asymp, Half Symp', log=True)
# ax.errorbar(x, mean_peak_H_list[6:9], yerr = y_upper_error[6:9], color = "k", capsize=3, fmt=".", ms = 0.0001)

# x = [i + width for i in x]
# plt.bar(x, mean_peak_H_list[9:12], width=width, label='Pure Asymp', log=True)
...
# plt.bar(x, mean_sum_asymptomatic_testing_list, label='Asymptomatic Testing')
# plt.bar(x, mean_sum_symptomatic_testing_list, bottom= mean_sum_asymptomatic_testing_list, label='Symptomatic Testing')
# plt.legend(loc=4)
...
# plt.ylabel('Total Testing Number')
