...
# mean_different_I_list = []
# upper_different_I_list = []

# mean_last_day_active_I_list = []
# upper_last_day_active_I_list = []

last_day_active_I_list = []

num_of_population_list = [50000, 100000, 150000, 200000]


for num_of_population in num_of_population_list:
    
    data_name = "output_num_population_{}.csv.process_all.csv".format(
        num_of_population
    )
    ...
    # I_difference_current = []
    # last_day_active_I_current = []
    ...
    for sample_run_count in range(sample_total_run_count):
        ...
        # I_difference_current.append(I[-1] - I[0])
...
for fix_testing_number in [-1, 0, 100, 200]:
    ...
    for total_population in [50000, 100000, 150000, 200000]:
        ...
        for run_count in range(sample_total_run_count):
            ...
            current_sample_data = current_sample_data[current_sample_data['total_population'] == total_population]
...
xtick = ["50000", "100000", "150000", "200000"]
...
plt.bar(x, mean_last_cum_I_list[4:8], width=width, label='Random', tick_label = xtick)
...
x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[12:16], width=width, label='Half Public, Half Symp')
ax.errorbar(x, mean_last_cum_I_list[12:16], yerr = y_upper_error[12:16], color = "k", capsize=3, fmt=".", ms = 0.0001)

x = [i + width for i in x]
plt.bar(x, mean_last_cum_I_list[16:20], width=width, label='Pure Public')
ax.errorbar(x, mean_last_cum_I_list[16:20], yerr = y_upper_error[16:20], color = "k", capsize=3, fmt=".", ms = 0.0001)

plt.legend()

plt.ylabel("Last Day Active I / Total Population")

plt.savefig("./Plots/active_I_bar.png")
plt.close(fig)
...
x = [i + width for i in x]
plt.bar(x, advantages_half_half, width=width, label='50-ASY', tick_label = xtick, hatch = patterns[5])
...
x = [i + width for i in x]
plt.bar(x, mean_peak_H_list[4:8], width=width, label='Random', tick_label = xtick)
ax.errorbar(x, mean_peak_H_list[4:8], yerr = y_upper_error[4:8], color = "k", capsize=3, fmt=".", ms = 0.0001)

x = [i + width for i in x]
plt.bar(x, mean_peak_H_list[8:12], width=width, label='Pure Symp')
ax.errorbar(x, mean_peak_H_list[8:12], yerr = y_upper_error[8:12], color = "k", capsize=3, fmt=".", ms = 0.0001)

x = [i + width for i in x]
plt.bar(x, mean_peak_H_list[12:16], width=width, label='Half Public, Half Symp')
ax.errorbar(x, mean_peak_H_list[12:16], yerr = y_upper_error[12:16], color = "k", capsize=3, fmt=".", ms = 0.0001)

x = [i + width for i in x]
plt.bar(x, mean_peak_H_list[12:16], width=width, label='Half Public, Half Symp')
ax.errorbar(x, mean_peak_H_list[12:16], yerr = y_upper_error[16:20], color = "k", capsize=3, fmt=".", ms = 0.0001)

plt.legend(loc = 4)

plt.ylabel("Peak H / Total Population")
...
sum_mean_asymptomatic_testing_list = [sum(x) for x in mean_asymptomatic_testing_list]
sum_upper_asymptomatic_testing_list = [sum(x) for x in upper_asymptomatic_testing_list]

sum_mean_symptomatic_testing_list = [sum(x) for x in mean_symptomatic_testing_list]
sum_upper_symptomatic_testing_list = [sum(x) for x in upper_symptomatic_testing_list]

x = list(range(len(sum_mean_asymptomatic_testing_list)))

plt.bar(x, sum_mean_asymptomatic_testing_list, label='Public Testing Number', tick_label = xtick)
plt.bar(x, sum_mean_symptomatic_testing_list, bottom= sum_mean_asymptomatic_testing_list, label='Symptomatic Testing Number')
plt.legend(loc=4)


y_upper_error = [ x-y for x,y in zip(sum_upper_asymptomatic_testing_list, sum_mean_asymptomatic_testing_list)]
ax.errorbar(x, sum_mean_asymptomatic_testing_list, yerr = y_upper_error, color = "k", capsize=3, fmt=".", ms = 0.0001)
