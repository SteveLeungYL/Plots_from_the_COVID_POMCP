...
def run_program_from_shell_command(total_population = 50000, num_of_simulation = 2 ** 12, output_file_name = "output.csv", horizon = 30, testing_action_step = 10, R0_value = 2.0, I_ratio = 0.0056, R_ratio = 0.0051, total_testing_number = 200, testing_sensitivity = 0.9, testing_group_number = 32):
    # Shell command space.
    shell_command = "./pomcp --problem covid --outputfile {} --total_population {} --horizon {} --maximum_testing_number {} --R0_value {:.2f} --I_class_ratio {:.4f} --R_class_ratio {:.4f} " \
                    "  --testing_action_step {} --runs 100 --mindoubles {} --maxdoubles {} --time_step 1 --testing_sensitivity {:.2f} --testing_group_number {}  " \
...
if __name__=="__main__":
    ...
    fixed_testing_sensitivity = 0.9

    fixed_testing_number = 500

    for testing_group_number in [16, 32]:

        print("Running on population: {}, total_testing_number:{}, num_simulation: {}, horizon: {}, testing_action_step: {}, R0_value: {:.2f}, I_ratio: {:.4f}, R_ratio: {:.4f}, testing_sensitivity: {:.2f}, testing_group_number: {}".format(
            fixed_population,
            ...
            fixed_I_ratio,
            fixed_R_ratio,
            fixed_testing_sensitivity,
            testing_group_number
        ))

        output_file_name = "output_group_number_{}.csv".format(testing_group_number)
        ...
        run_program_from_shell_command(total_population=fixed_population, num_of_simulation=fixed_number_of_simulation,
                                       ...
                                       total_testing_number = fixed_testing_number, testing_sensitivity = fixed_testing_sensitivity, testing_group_number=testing_group_number)
